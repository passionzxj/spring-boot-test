...
public class JwtTokenUtil implements Serializable {
    ...
    public Boolean validateToken(String token, JwtUser userDetails) {
        ...
        return (username.equals(userDetails.getAccount()) && !isTokenExpired(token));
    }
     ...
     * 根据token获取map
     */
    public static Map<String, String> getUsernameFromToken2(String token) {
        Map map = new HashMap();
        Claims fromToken = getClaimsFromToken(token);
        map.put("account", fromToken.get(CLAIM_KEY_USERNAME));
        return map;
    }
